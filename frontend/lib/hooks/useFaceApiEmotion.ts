...
/** Loaded once — avoid dynamic import() on every tick (was a major perf hit). */
...
export function useFaceApiEmotion(
  ...
  useEffect(() => {
    ...
    getFaceApi().then(async (faceapi) => {
      try {
        const api = (faceapi as { default?: typeof faceapi }).default ?? faceapi;
        await Promise.all([
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(async () => {
      ...
      try {
        const mod = await getFaceApi();
        const faceapi = (mod as { default?: typeof mod }).default ?? mod;
